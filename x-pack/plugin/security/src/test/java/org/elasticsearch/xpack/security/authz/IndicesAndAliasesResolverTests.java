...
public class IndicesAndAliasesResolverTests extends ESTestCase {
    ...
    public void setup() {
        ...
        doAnswer((i) -> {
            ...
            for (String name : namedRoleReference.roleNames()) {
    ...
    public void testExplicitDashIndices() {
        ...
        List<String> indices = resolveIndices(request, buildAuthorizedIndices(userDashIndices, SearchAction.NAME)).local();
    ...
    public void testResolveEmptyIndicesExpandWilcardsOpenAndClosed() {
        ...
        List<String> indices = resolveIndices(request, buildAuthorizedIndices(user, SearchAction.NAME)).local();
    ...
    public void testResolveWildcardsNoExpand() {
        ...
        assertThat(indices.local(), containsInAnyOrder(replacedIndices));
        assertThat(request.indices(), arrayContainingInAnyOrder(replacedIndices));
    ...
    public void testSearchWithRemoteIndex() {
        ...
        assertThat(resolved.local(), emptyIterable());
        assertThat(resolved.remote(), containsInAnyOrder("remote:indexName"));
    ...
    public void testSearchWithRemoteAndLocalIndices() {
        ...
        request.indicesOptions(IndicesOptions.fromOptions(true, randomBoolean(), randomBoolean(), randomBoolean()));
        final ResolvedIndices resolved = resolveIndices(request, buildAuthorizedIndices(user, SearchAction.NAME));
        assertThat(resolved.local(), containsInAnyOrder("bar"));
        assertThat(resolved.remote(), containsInAnyOrder("remote:indexName"));
    ...
    public void testSearchWithRemoteAndLocalWildcards() {
        ...
        assertThat(resolved.remote(), containsInAnyOrder("remote:foo", "other_remote:foo", "remote:bar*", "remote:baz*"));
        assertThat(resolved.local(), containsInAnyOrder("bar", "foofoo"));
        assertThat(
            request.indices(),
    ...
    public void testResolveIndicesAliasesRequest() {
        ...
        List<String> indices = resolveIndices(request, buildAuthorizedIndices(user, IndicesAliasesAction.NAME)).local();
    ...
    public void testResolveIndicesAliasesRequestDeleteActions() {
        ...
        List<String> indices = resolveIndices(request, authorizedIndices).local();
        // the union of all indices and aliases gets returned
    ...
    public void testGetAliasesRequestMissingIndexIgnoreUnavailableDisallowNoIndices() {
        ...
        IndexNotFoundException exception = expectThrows(
            IndexNotFoundException.class,
            () -> resolveIndices(request, buildAuthorizedIndices(user, GetAliasesAction.NAME)).local()
        );
    ...
    public void testResolveAliasesAllGetAliasesRequestNoAuthorizedIndices() {
        ...
        assertThat(resolvedIndices.local(), contains("non_existing"));
    ...
    public void testRemotableRequestsAllowRemoteIndices() {
        ...
        assertThat(resolved.remote(), containsInAnyOrder("remote:foo"));
        assertThat(resolved.local(), emptyIterable());
    ...
    public void testNonRemotableRequestDoesNotAllowRemoteIndices() {
        ...
        IndexNotFoundException e = expectThrows(
            IndexNotFoundException.class,
            () -> resolveIndices(tuple.v1(), buildAuthorizedIndices(user, tuple.v2())).local()
        );
    ...
    public void testXPackSecurityUserHasAccessToSecurityIndex() {
        ...
        }
        {
            ...
            List<String> indices = resolveIndices(aliasesRequest, authorizedIndices).local();
            assertThat(indices, hasItem(SECURITY_MAIN_ALIAS));
        }
    }
    ...
    public void testAliasDateMathExpressionNotSupported() {
        ...
        List<String> indices = resolveIndices(request, buildAuthorizedIndices(user, GetAliasesAction.NAME)).local();
        // the union of all indices and aliases gets returned
    ...
    public void testHiddenIndicesResolution() {
        ...
        assertThat(resolvedIndices.remote(), emptyIterable());
        ...
        resolvedIndices = defaultIndicesResolver.resolveIndicesAndAliases(SearchAction.NAME, searchRequest, metadata, authorizedIndices);
        assertThat(resolvedIndices.local(), containsInAnyOrder(".hidden-open"));
        assertThat(resolvedIndices.remote(), emptyIterable());
        ...
        authorizedIndices = buildAuthorizedIndices(user, SearchAction.NAME);
        resolvedIndices = defaultIndicesResolver.resolveIndicesAndAliases(SearchAction.NAME, searchRequest, metadata, authorizedIndices);
        assertThat(resolvedIndices.local(), containsInAnyOrder("bar-closed", "foofoo-closed", "hidden-closed", ".hidden-closed"));
        assertThat(resolvedIndices.remote(), emptyIterable());
        ...
        authorizedIndices = buildAuthorizedIndices(user, SearchAction.NAME);
        resolvedIndices = defaultIndicesResolver.resolveIndicesAndAliases(SearchAction.NAME, searchRequest, metadata, authorizedIndices);
        assertThat(resolvedIndices.local(), containsInAnyOrder(".hidden-closed"));
        assertThat(resolvedIndices.remote(), emptyIterable());
        ...
        authorizedIndices = buildAuthorizedIndices(user, SearchAction.NAME);
        resolvedIndices = defaultIndicesResolver.resolveIndicesAndAliases(SearchAction.NAME, searchRequest, metadata, authorizedIndices);
        assertThat(resolvedIndices.local(), contains("-*"));
        assertThat(resolvedIndices.remote(), emptyIterable());
        ...
        authorizedIndices = buildAuthorizedIndices(user, SearchAction.NAME);
        resolvedIndices = defaultIndicesResolver.resolveIndicesAndAliases(SearchAction.NAME, searchRequest, metadata, authorizedIndices);
        assertThat(resolvedIndices.local(), contains(oneOf("date-hidden-" + todaySuffix, "date-hidden-" + tomorrowSuffix)));
        assertThat(resolvedIndices.remote(), emptyIterable());
    }
    ...
    public void testHiddenAliasesResolution() {
        ...
        assertThat(resolvedIndices.local(), containsInAnyOrder("alias-visible", "alias-visible-mixed"));
        assertThat(resolvedIndices.remote(), emptyIterable());
        ...
        searchRequest.indicesOptions(IndicesOptions.fromOptions(false, false, true, false, true));
        resolvedIndices = defaultIndicesResolver.resolveIndicesAndAliases(SearchAction.NAME, searchRequest, metadata, authorizedIndices);
        assertThat(resolvedIndices.local(), containsInAnyOrder("alias-hidden"));
        assertThat(resolvedIndices.remote(), emptyIterable());
    ...
    public void testDataStreamResolution() {
        {
            ...
            assertThat(resolvedIndices.local(), contains("logs-foobar"));
            assertThat(resolvedIndices.remote(), emptyIterable());
        ...
        }
        {
            ...
            assertThat(resolvedIndices.local(), containsInAnyOrder("logs-foo", "logs-foobar"));
            assertThat(resolvedIndices.remote(), emptyIterable());
        }
    }
    ...
    public void testDataStreamsAreNotVisibleWhenNotIncludedByRequestWithWildcard() {
        ...
        for (String dsName : dataStreams) {
            assertThat(resolvedIndices.local(), hasItem(dsName));
            DataStream dataStream = metadata.dataStreams().get(dsName);
            assertThat(resolvedIndices.local(), hasItem(dsName));
            for (Index i : dataStream.getIndices()) {
                assertThat(resolvedIndices.local(), hasItem(i.getName()));
            }
        }
    }
    ...
    public void testDataStreamsAreNotVisibleWhenNotIncludedByRequestWithoutWildcard() {
        ...
        assertThat(resolvedIndices.local(), hasItem(dataStreamName));
        for (Index i : dataStream.getIndices()) {
            assertThat(resolvedIndices.local(), hasItem(i.getName()));
        }
    }
    ...
    public void testDataStreamsAreVisibleWhenIncludedByRequestWithWildcard() {
        ...
        assertThat(resolvedIndices.local(), hasItem("logs-foo"));
        assertThat(resolvedIndices.local(), hasItem("logs-foobar"));
        assertThat(resolvedIndices.local(), hasItem("logs-00001"));
        assertThat(resolvedIndices.local(), hasItem("logs-00002"));
        assertThat(resolvedIndices.local(), hasItem("logs-00003"));
        assertThat(resolvedIndices.local(), hasItem("logs-alias"));
        for (String dsName : expectedDataStreams) {
            DataStream dataStream = metadata.dataStreams().get(dsName);
            assertNotNull(dataStream);
            for (Index i : dataStream.getIndices()) {
                assertThat(resolvedIndices.local(), not(hasItem(i.getName())));
            }
        }
    }
    ...
    public void testBackingIndicesAreVisibleWhenIncludedByRequestWithWildcard() {
        ...
        for (String dsName : expectedDataStreams) {
            DataStream dataStream = metadata.dataStreams().get(dsName);
            assertThat(resolvedIndices.local(), not(hasItem(dsName)));
            for (Index i : dataStream.getIndices()) {
                assertThat(resolvedIndices.local(), hasItem(i.getName()));
            }
        }
    ...
    public void testDataStreamNotAuthorizedWhenBackingIndicesAreAuthorizedViaWildcardAndRequestThatIncludesDataStreams() {
        ...
        assertThat(resolvedIndices.local(), not(hasItem(dataStream.getName())));
        for (Index i : dataStream.getIndices()) {
            assertThat(authorizedIndices, hasItem(i.getName()));
        }
    }
    ...
    public void testDataStreamNotAuthorizedWhenBackingIndicesAreAuthorizedViaNameAndRequestThatIncludesDataStreams() {
        ...
        assertThat(resolvedIndices.local(), not(hasItem("logs-foobar")));
        assertThat(resolvedIndices.local(), contains(DataStream.getDefaultBackingIndexName("logs-foobar", 1)));
    }
    ...
    private static void assertNoIndices(IndicesRequest.Replaceable request, ResolvedIndices resolvedIndices) {
        final List<String> localIndices = resolvedIndices.local();
        ...
        assertEquals(0, resolvedIndices.remote().size());
    }
